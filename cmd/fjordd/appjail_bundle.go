...
func directorEnv(envDefaults, stackID string, resolvedEnv, volPlaceholders map[string]string, allVolVars map[string]bool) string {
	...
	// TZ is a fact about the host, not something the wizard asks for, so
	// nothing filled it and it fell out of the file below as an empty value.
	// The director hands ${TZ} to every service, and an empty one is worse
	// than a missing one: immich's server schedules cron jobs at boot, and
	// cron refuses an empty zone with "You specified an invalid date" -- a
	// crash loop three layers from anything that mentions time.
	if env["TZ"] == "" {
	...
	for _, k := range order {
		// A key the bundle declared is written even with no value. Dropping it
		// did not make it go away -- director still substitutes ${KEY} into
		// every service that names it, just with nothing -- and it left the
		// operator no line to fill in, because the key was invisible in the
		// file they were told to edit.
		fmt.Fprintf(&b, "%s=%s\n", k, env[k])
	}
	return b.String()
}

// hostTimezone is the zone this host keeps, in the tzdata name a container
// expects ("America/New_York"). "" when it cannot be read, which leaves the
// bundle's own default alone rather than inventing UTC for a host that is not
// on it.
func hostTimezone() string {
	// FreeBSD records the chosen zone here verbatim; tzsetup writes it.
	...
	// Elsewhere /etc/localtime is a symlink into the zoneinfo tree, and the
	// name is the part below it.
	if dest, err := os.Readlink("/etc/localtime"); err == nil {
		if _, rest, ok := strings.Cut(dest, "/zoneinfo/"); ok && rest != "" {
			return rest
		}
	}
	return ""
