...
func writeAppjailBundle(dir, stackID string, b *manifest.AppjailBundle, resolvedEnv map[string]string, composeYAML, network, ip string) (string, error) {
	if b.Director == "" {
	...
	if network != "" {
		directorYML, err = setDirectorNetwork(directorYML, stackID, network, ip)
		if err != nil {
